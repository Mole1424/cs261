...
parser = argparse.ArgumentParser(description='Console interface for test suite. Running without options will run a default set of tests.')
parser.add_argument('-l', '--list', action='store_true', help='show list of all available tests and exit')
parser.add_argument('-q', '--quiet', action='store_true', help='silences test announcements')
parser.add_argument('-t', '--test', action='append', nargs='+', choices=available_tests, help='select specific tests to run') # nargs='+' allows for 1 or more arguments
parser.add_argument('-v', '--verbose', action='store_true', help='prints results of passed tests')
